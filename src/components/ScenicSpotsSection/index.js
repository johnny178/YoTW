...
const ScenicSpotsSection = () => {
  ...
  const setFilterName = useCallback(() => {
    ...
    if ((regionSelect !== '全部地區' && countrySelect === '全台')) {
      name += searchValue.length !== 0 ? ` and contains(ScenicSpotNAME,'${searchValue}') and (` : ' and (';
      regionTaiwan[regionSelect].slice(1).map((region, index) => {
        name += (index !== 0 ? ' or ' : '') + `contains(City,'${region}')`;
      });
      name += ')';
    } else {
      name += searchValue.length !== 0 ? ` and contains(ScenicSpotNAME,'${searchValue}')` : '';
    }
    return name;
  ...
  const renderScenicSpots = () => (
    scenicSpots.map((item, index) => {
      if (item?.Picture?.PictureUrl1?.includes('210.69') ||
        ...
        item?.ScenicSpotName?.includes('Test')
      )
        return;
      return (
        <ResultItem
